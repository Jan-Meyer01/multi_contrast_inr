...
def resample_nib(img, voxel_spacing=(1, 1, 1), order=3):
    ...
    new_shp = tuple(np.rint([
        shp[0] / voxel_spacing[0],
        shp[1] / voxel_spacing[1],
        shp[2] / voxel_spacing[2]
        ]).astype(int))
    
    """
    # old downsampling that considers voxel spacing, but messes up the size of the tensors
    ...
    new_voxel_spacing = [img.header.get_zooms()[i] * voxel_spacing[i] for i in range(3)]
    
    # and resample the volume
    new_aff = nib.affines.rescale_affine(aff, shp, new_voxel_spacing, new_shp)
